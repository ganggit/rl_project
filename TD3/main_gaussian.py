...
import utils
from TD3_Gaussian_m import TD3
import OurDDPG
import DDPG
import collections
import pickle
...
def evaluate_policy(policy, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            action, log_pi = policy.select_action(np.array(obs))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_name", default='Ant-v2')#default='BipedalWalker-v3')
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if done:
            ...
            dataSaved.append(
                Data(steps=episode_timesteps, reward=episode_reward))
            # Reset environment
            obs = env.reset()
            done = False
            episode_reward = 0
        ...
        replay_buffer.add((obs, new_obs, action, reward, log_pi, done_bool))
        ...
        if(total_timesteps > 100000 or total_timesteps%100):
            with open(file_name+'.pkl', 'wb') as handle:
                pickle.dump(dataSaved, handle)
    # Final evaluation
    evaluations.append(evaluate_policy(policy))
